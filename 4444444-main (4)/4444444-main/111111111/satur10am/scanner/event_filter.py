...
This module implements a lean, event-driven pipeline that monitors Swap
logs for a curated set of pools and only surfaces pools whose price moved
significantly. The goal is to drastically reduce CPU/RPC load compared to
brute-force pair iteration by waking the arbitrage engine only when the
chain tells us something interesting happened.

Design highlights
-----------------
* Async, non-blocking polling of ``eth_getLogs`` (WS-ready hook points).
* Subscription set derived from registry liquidity/core-token heuristics.
* Price-change scoring without extra RPC calls (uses cached reserves).
* Deduplication window to avoid flooding the engine with repeat swaps.
* Bounded candidate queue with clear logging and safe fallbacks.

The filter is intended to be used by higher-level strategy orchestration
(e.g. :class:`core.arbitrage_engine.ArbitrageEngine`).  If disabled in
config it cleanly no-ops and the caller can fall back to brute force.
"""
...
@dataclass
class CandidatePool:
    """Pool that recently experienced a meaningful price move."""
...
class EventDrivenFilter:
    ...
    The filter polls (or subscribes) for Swap logs on a pre-selected
    subset of pools. When a Swap produces a price change above the
    configured threshold, the pool is enqueued as a candidate for the
    arbitrage engine to validate.
    """
    ...
    def get_candidates(self, max_items: int | None = None) -> List[CandidatePool]:
        ...
        ----------
        max_items:
            Optional hard limit of candidates to return; defaults to
